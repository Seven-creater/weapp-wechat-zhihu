 ...
 *   type: 'collect_solution' | 'collect_post', // 收藏类型
...
const checkIsCollected = function (type, targetId) {
  return new Promise((resolve, reject) => {
    // 在微信云开发中，_openid 由系统自动处理
    // 查询时不需要指定 _openid，系统会自动过滤当前用户的数据
...
const getCollectCount = function (type, targetId) {
  return new Promise((resolve, reject) => {
    // 这里需要根据实际情况从目标集合获取收藏数量
    // 暂时返回0，后续需要根据具体业务逻辑实现
    resolve(0);
  });
};
    ...
    checkLogin()
      .then((userInfo) => {
        ...
        return checkIsCollected(type, targetId).then((isCollected) => {
          ...
          const newCount = newStatus ? currentCount + 1 : currentCount - 1;
...
const removeFromCollection = function (openid, type, targetId) {
  return new Promise((resolve, reject) => {
    // 在微信云开发中，_openid 由系统自动处理
    // 删除时不需要指定 _openid，系统会自动过滤当前用户的数据
    db.collection("actions")
      .where({
        type: type,
        targetId: targetId,
