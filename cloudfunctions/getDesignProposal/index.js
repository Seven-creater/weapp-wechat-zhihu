...
const _ = db.command;
const media = require('../getDesignProposals/media');

exports.main = async (event = {}) => {
  try {
    const proposalId = toSafeString(event.proposalId || event.id, 64);
    const issueId = toSafeString(event.issueId, 64);
    if (!proposalId && !issueId) {
      return {
        success: false,
    ...
    let query = db.collection('design_proposals')
      .field({
        _id: true,
        _openid: true,
        postId: true,
    ...
    if (proposalId) {
      query = query.where({ _id: proposalId }).limit(1);
    } else {
      query = query.where(_.or([{ issueId }, { postId: issueId }]))
    ...
    const designerMap = await fetchDesignerMap([toSafeString(row.designerId || row._openid, 64)]);
    const normalized = normalizeProposalDoc(row, designerMap);
    ...
    console.error('[getDesignProposal] failed:', err);
    return {
      success: false,
...
async function fetchDesignerMap(openids) {
  ...
  const avatarMap = await media.resolveTempUrlMap(cloud, Array.from(avatarIds), {
    scenario: 'getDesignProposal.designerAvatar'
  });
